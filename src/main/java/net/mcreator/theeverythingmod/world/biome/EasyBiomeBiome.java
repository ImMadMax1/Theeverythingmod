...
package net.mcreator.theeverythingmod.world.biome;
...
import net.mcreator.theeverythingmod.block.EasyLeavesAppleBlock;
import net.mcreator.theeverythingmod.block.EasyGrassBlock;
...
public class EasyBiomeBiome extends TheEverythingModModElements.ModElement {
	@ObjectHolder("the_everything_mod:easy_biome")
	public static final CustomBiome biome = null;
	public EasyBiomeBiome(TheEverythingModModElements instance) {
		super(instance, 19);
	}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == EasyGrassBlock.block.getDefaultState().getBlock() || ground == EasystoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == EasyGrassBlock.block.getDefaultState().getBlock()
									|| ground2 == EasystoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), EasystoneBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == EasyLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, EasyLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, EasyOakLogBlock.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												EasyLeavesAppleBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == EasyOakLogBlock.block.getDefaultState().getBlock()
					|| blockType == EasyLeavesBlock.block.getDefaultState().getBlock()
					|| blockType == EasyGrassBlock.block.getDefaultState().getBlock()
					|| blockType == EasystoneBlock.block.getDefaultState().getBlock();
		}
